...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet =
    ...
    else that.union(tail).incl(head)
  ...
  def ascendingByRetweet0(acc: Trending): Trending =
    if (isEmpty) acc
    else remove(findMin).ascendingByRetweet0(acc + findMin)
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = containedIn(google)

  val appleTweets: TweetSet = containedIn(apple)

  def containedIn(list: List[String]) = 
    TweetReader.allTweets.filter(t => list.exists(t.text.contains))
